...
def compute_gae(rewards: torch.Tensor,
    ...
    Compute GAE advantages.
      rewards: [T,] or [T,] tensor of rewards
      values:  [T+1,] tensor of value estimates (note the extra bootstrap value)
      dones:   [T,] binary mask (1 if done after step t, else 0)
    Returns:
      advantages: [T,] tensor
    """
    T = rewards.shape[0]
...
def compute_group_relative_advantages(rewards: torch.Tensor,
                                      values: torch.Tensor,
                                      dones: torch.Tensor,
    ...
    2) Subtract each group's mean advantage so group-relative advantages sum to zero
    
    rewards:   [T,] tensor
    values:    [T+1,] tensor
    dones:     [T,] tensor of 0/1
    group_ids: [T,] integer tensor (e.g. batch element → group index)
    """
    # 1) GAE
